...
class BeritaController extends Controller
{
    ...
    {
        $berita = News::published()->where('slug', $slug)->first();

        // Mock data per slug jika tidak ada di DB
        if (!$berita) {
            $mockData = [
                'beasiswa-daerah-3t' => [
            ...
            $data = $mockData[$slug] ?? null;

            $berita = new News();
            $berita->id = 0;
            $berita->slug = $slug;
            $berita->title    = $data['title']    ?? ucwords(str_replace('-', ' ', $slug));
            $berita->excerpt  = $data['excerpt']  ?? 'Lazismu NTB terus bergerak memberikan manfaat nyata bagi masyarakat NTB.';
            $berita->content  = $data['content']  ?? '<p>Konten berita akan segera tersedia.</p>';
            $berita->image    = $data['image']    ?? null;
            $berita->category = $data['category'] ?? 'Berita';
            $berita->published_at = isset($data['date']) ? \Carbon\Carbon::parse($data['date']) : now();
        }
