...
@section('title', 'Donasi Berhasil - Lazismu NTB')
        ...
        <div class="card success-card border-0 shadow-lg p-4 p-md-5 text-center">
            ...
            <div class="receipt-box text-start p-4 mb-4 rounded-3 border">
                ...
                <div class="receipt-row d-flex justify-content-between py-2 border-bottom">
                    ...
                    <strong class="text-dark small">{{ $orderId }}</strong>
                </div>

                @if($donation)
                <div class="receipt-row d-flex justify-content-between py-2 border-bottom">
                    <span class="text-muted small">Program</span>
                    <strong class="text-dark text-end small" style="max-width: 250px;">{{ $donation->program->title }}</strong>
                </div>

                <div class="receipt-row d-flex justify-content-between py-2 border-bottom">
                    <span class="text-muted small">Nama Donatur</span>
                    <strong class="text-dark small">{{ $donation->donor_name }}</strong>
                </div>

                <div class="receipt-row d-flex justify-content-between py-2 border-bottom">
                    <span class="text-muted small">Metode Pembayaran</span>
                    <strong class="text-dark small">{{ strtoupper(str_replace('_', ' ', $donation->payment_method)) }}</strong>
                </div>

                <div class="receipt-row d-flex justify-content-between py-2 border-bottom">
                    <span class="text-muted small">Tanggal</span>
                    <strong class="text-dark small">{{ $donation->paid_at ? $donation->paid_at->format('d M Y, H:i') : now()->format('d M Y, H:i') }} WITA</strong>
                </div>

                <div class="receipt-row d-flex justify-content-between py-2 border-bottom">
                    <span class="text-muted small">Jumlah Donasi</span>
                    <strong class="text-dark small">Rp {{ number_format($donation->amount, 0, ',', '.') }}</strong>
                </div>

                @if($donation->admin_fee > 0)
                <div class="receipt-row d-flex justify-content-between py-2 border-bottom">
                    <span class="text-muted small">Biaya Admin</span>
                    <strong class="text-dark small">Rp {{ number_format($donation->admin_fee, 0, ',', '.') }}</strong>
                </div>
                @endif

                @if(!empty($donation->doa))
                <div class="receipt-row py-2 mt-3 border-top">
                    <span class="text-muted small d-block mb-2">Doa / Dukungan</span>
                    <div class="text-dark small" style="white-space: pre-wrap;">{{ $donation->doa }}</div>
                </div>
                @endif

                <div class="receipt-row d-flex justify-content-between py-2 mt-2">
                    <span class="text-muted fw-bold">Total Dibayar</span>
                    <h4 class="text-green fw-extrabold mb-0">Rp {{ number_format($donation->amount + $donation->admin_fee, 0, ',', '.') }}</h4>
                </div>
                @endif
            </div>
            ...
            <div class="d-flex flex-column gap-2">
                <a href="{{ route('home') }}" class="btn btn-primary btn-lg py-3 fw-bold rounded-3">
                    Kembali ke Beranda
                </a>
